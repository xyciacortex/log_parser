...
class LogParserApp:
    ...
    def get_log_severity(self, row_values):
        ...
        # 🔹 Define additional error keywords
        error_keywords = ["error", "failed", "critical", "exception", "denied"]
        warning_keywords = ["warning", "deprecated", "slow", "retry"]
        
        # 🔥 Add custom check for Okta domain issue
        if "okta domain should not contain -admin" in message or "okta.com" in message and "-admin" in message:
            return "error"  # Treat as an error
